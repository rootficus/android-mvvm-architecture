...
class DashBoardRepository (val apiServices: FionApiServices,
                           val context: Context,
    ...
    fun getBalanceManageRecord(isBalanceManage : Int) : ArrayList<GetBalanceManageRecord> {
        return when(isBalanceManage){
            -1 -> fionDatabase.rapidxDao()?.getBalanceTransaction() as ArrayList<GetBalanceManageRecord>
            Constant.BalanceManagerStatus.SUCCESS.action -> {
                ...
                fionDatabase.rapidxDao()?.getBalanceTransaction(Constant.BalanceManagerStatus.SUCCESS.toString()) as ArrayList<GetBalanceManageRecord>
            }
            Constant.BalanceManagerStatus.PENDING.action -> {
                ...
                fionDatabase.rapidxDao()?.getBalanceTransaction(Constant.BalanceManagerStatus.PENDING.toString()) as ArrayList<GetBalanceManageRecord>
            }
            Constant.BalanceManagerStatus.APPROVED.action -> {
                ...
                fionDatabase.rapidxDao()?.getBalanceTransaction(Constant.BalanceManagerStatus.APPROVED.toString()) as ArrayList<GetBalanceManageRecord>
            }
            Constant.BalanceManagerStatus.REJECTED.action -> {
                ...
                fionDatabase.rapidxDao()?.getBalanceTransaction(Constant.BalanceManagerStatus.REJECTED.toString()) as ArrayList<GetBalanceManageRecord>
            }
            Constant.BalanceManagerStatus.DANGER.action -> {
                ...
                fionDatabase.rapidxDao()?.getBalanceTransaction(Constant.BalanceManagerStatus.DANGER.toString()) as ArrayList<GetBalanceManageRecord>
            }
            else -> {
                Log.d("getBalanceManagerRecord",":else:${isBalanceManage},${Constant.BalanceManagerStatus.DANGER.toString()}")
                fionDatabase.rapidxDao()?.getBalanceTransaction() as ArrayList<GetBalanceManageRecord>
            }
        }
    }
    ...
    fun getCountBalanceManageRecord(isBalanceManage : Int) : Int {
        return when(isBalanceManage){
            ...
            Constant.BalanceManagerStatus.SUCCESS.action -> fionDatabase.rapidxDao()?.getCountBalanceTransaction(Constant.BalanceManagerStatus.SUCCESS.toString()) as Int
            Constant.BalanceManagerStatus.PENDING.action -> fionDatabase.rapidxDao()?.getCountBalanceTransaction(Constant.BalanceManagerStatus.PENDING.toString()) as Int
            Constant.BalanceManagerStatus.APPROVED.action -> fionDatabase.rapidxDao()?.getCountBalanceTransaction(Constant.BalanceManagerStatus.APPROVED.toString()) as Int
            Constant.BalanceManagerStatus.REJECTED.action -> fionDatabase.rapidxDao()?.getCountBalanceTransaction(Constant.BalanceManagerStatus.REJECTED.toString()) as Int
            Constant.BalanceManagerStatus.DANGER.action -> fionDatabase.rapidxDao()?.getCountBalanceTransaction(Constant.BalanceManagerStatus.DANGER.toString()) as Int
            else -> {
    ...
    fun getMessageManageRecord(isBalanceManage : Int) : ArrayList<GetMessageManageRecord> {
        return when(isBalanceManage){
            ...
            -1 -> fionDatabase.rapidxDao()?.getMessageTransaction() as ArrayList<GetMessageManageRecord>
            Constant.SMSType.CashOut.value -> {
                Log.d("getMessageManageRecord",":CashOut:${isBalanceManage},${Constant.SMSType.CashOut.value}")
                fionDatabase.rapidxDao()?.getMessageTransaction(Constant.SMSType.CashOut.value) as ArrayList<GetMessageManageRecord>
            }
            Constant.SMSType.CashIn.value -> {
                Log.d("getMessageManageRecord",":CashIn:${isBalanceManage},${Constant.SMSType.CashIn.value}")
                fionDatabase.rapidxDao()?.getMessageTransaction(Constant.SMSType.CashIn.value) as ArrayList<GetMessageManageRecord>
            }
            Constant.SMSType.B2B.value -> {
                Log.d("getMessageManageRecord",":B2B:${isBalanceManage},${Constant.SMSType.B2B.value}")
                fionDatabase.rapidxDao()?.getMessageTransaction(Constant.SMSType.B2B.value) as ArrayList<GetMessageManageRecord>
            }
            Constant.SMSType.UNKNOWN.value -> {
                Log.d("getMessageManageRecord",":UNKNOWN:${isBalanceManage},${Constant.SMSType.UNKNOWN.value}")
                fionDatabase.rapidxDao()?.getMessageTransaction(Constant.SMSType.UNKNOWN.value) as ArrayList<GetMessageManageRecord>
            }
            else -> {
                Log.d("getMessageManageRecord",":AllElse:${isBalanceManage},${Constant.SMSType.All.value}")
                fionDatabase.rapidxDao()?.getMessageTransaction() as ArrayList<GetMessageManageRecord>
            }
        }
    }
    ...
    fun getCountMessageManageRecord(isBalanceManage : Int) : Int {
        return when(isBalanceManage){
            ...
            Constant.SMSType.CashOut.value  -> fionDatabase.rapidxDao()?.getCountMessageTransaction(Constant.SMSType.CashOut.value) as Int
            Constant.SMSType.CashIn.value  -> fionDatabase.rapidxDao()?.getCountMessageTransaction(Constant.SMSType.CashIn.value) as Int
            Constant.SMSType.B2B.value  -> fionDatabase.rapidxDao()?.getCountMessageTransaction(Constant.SMSType.B2B.value) as Int
            Constant.SMSType.UNKNOWN.value  -> fionDatabase.rapidxDao()?.getCountMessageTransaction(Constant.SMSType.UNKNOWN.value) as Int
            else -> {
